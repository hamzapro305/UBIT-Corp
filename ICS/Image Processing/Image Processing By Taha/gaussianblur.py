...
img = open(f"C:\\Users\\Hp\\Downloads\\python_images\\HouseStarkShield1.BMP", "r+b")
img.seek(18, 0)
...
for r in range(height):
    for c in range(width):
        if (r == 0 or r == height - 1) or (c == 0 or c == width - 1):
            ...
            b = int(floor(blueconv)).to_bytes(1, "little")
            g = int(floor(greenconv)).to_bytes(1, "little")
            r_ed = int(floor(redconv)).to_bytes(1, "little")
            img.write(b)
            img.write(g)
            img.write(r_ed)
